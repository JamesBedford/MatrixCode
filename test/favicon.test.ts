...
describe("buildFaviconSvg", () => {
  it("draws ten katakana glyphs across three columns, one glowing head each", () => {
    const svg = buildFaviconSvg(getPreset("classic"));
    expect(count(svg, "<text")).toBe(10);
  ...
  it("colours the rain from the preset: body trail, bright body, white head, themed background", () => {
    const svg = buildFaviconSvg(getPreset("classic"));

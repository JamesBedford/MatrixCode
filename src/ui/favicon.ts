...
interface Cell {
  ...
  y: number;
  glyph: string;
  /** Index into the brightness ramp (0 = dim trail … 3 = bright head). */
  level: number;
}

// Three staggered columns, head (level 3) leading at the bottom of each — matching the rain's
// stationary-grid look where a wave of illumination sweeps down and decays up the trail.
const CELLS: readonly Cell[] = [
  // column 1
  { x: 14, y: 10.5, glyph: "ﾊ", level: 0 },
  { x: 14, y: 22.5, glyph: "ｼ", level: 1 },
  { x: 14, y: 34.5, glyph: "ﾘ", level: 2 },
  { x: 14, y: 46.5, glyph: "ﾂ", level: 3 },
  // column 2
  { x: 32, y: 34.5, glyph: "ｦ", level: 0 },
  { x: 32, y: 46.5, glyph: "ﾐ", level: 1 },
  { x: 32, y: 58.5, glyph: "ﾝ", level: 3 },
  // column 3
  { x: 50, y: 10.5, glyph: "ﾅ", level: 1 },
  { x: 50, y: 22.5, glyph: "ｴ", level: 2 },
  { x: 50, y: 34.5, glyph: "ｱ", level: 3 },
];
...
/** Build the favicon SVG markup for a preset. The ramp keeps the trail visible (body, not the
 *  near-black tail) and ends on the white-green head, so every theme reads as glowing rain. */
export function buildFaviconSvg(preset: ColorPreset): string {
  const ramp: Rgb[] = [preset.body, preset.bright, mix(preset.bright, preset.head, 0.45), preset.head];
  const glyphs = CELLS.map((c) => {
    const filter = c.level === 3 ? ' filter="url(#head-glow)"' : "";

...
- **Settings panel** — press `H` to toggle; controls for color theme, quality tier, glyph scale, and more. The panel and the native settings windows follow the active colour theme, including a calendar override.
...
- **Full-moon colors** — White for the whole local calendar day containing the full moon, using the same offline lunar calculation as `{countdown:fullmoon}`. Fixed holiday colors take priority if the dates coincide; the selected theme returns afterward. While either calendar override is active, settings show “Overridden by Valentine's Day”, “Overridden by St. Patrick's Day”, or “Overridden by full moon” beside the colour theme. The saved selection is left unchanged.
- **Occasion greetings** — on Valentine's Day, St Patrick's Day, Christmas Day, and a full-moon day, the rain shows “happy valentine's day!”, “happy st. patrick's day”, “happy christmas”, or “happy full moon” as the first in-rain message, using the configured glyph font. It appears even when messages are turned off, and it does not change the saved message list. A fixed holiday wins when it shares a day with a full moon.

...
Reduced-motion / no-intro paths never enter a choreography (the sentinel keeps `f = 1`), so they render the warmed-up rain exactly as today. The super-fullscreen path is untouched (it has no intro).
